...
def do_define_form(expressions, env):
    ...
    if scheme_symbolp(signature):
        ...
        # BEGIN PROBLEM 10
        define_formals = signature.rest
        validate_formals(define_formals)
        symbol = signature.first
        body = expressions.rest
        procedure = LambdaProcedure(define_formals,body,env)
...
def do_and_form(expressions, env):
    ...
    while current_exp.rest is not nil:
        eval_val = scheme_eval(current_exp.first, env)
        if is_scheme_false(eval_val):
            return eval_val
        current_exp = current_exp.rest
    return scheme_eval(current_exp.first,env,True)
